...
export function validatePoster(poster: Poster, options: { hasVisualFlowImage?: boolean } = {}): string[] {
  const errors: string[] = []
  ...
  if (!(options.hasVisualFlowImage ?? Boolean(c.visual_flow?.image))) errors.push('Attach one Visual Flow image.')
  ...
  const prose = c.core.bullets.map((bullet) => bullet.text).map(normalize).filter(Boolean)
