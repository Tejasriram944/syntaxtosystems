...
export const api = {
  ...
  create: (title: string, content: PosterContent) => request<Poster>('/posters', { method: 'POST', body: JSON.stringify({ title, content }) }),
  update: (poster: Poster) => request<Poster>(`/posters/${poster.id}`, { method: 'PATCH', body: JSON.stringify({ title: poster.title, content: poster.content, revision: poster.revision }) }),
  duplicate: (id: string) => request<Poster>(`/posters/${id}/duplicate`, { method: 'POST' }),
